...
for x in participants:
    ...
    for sessions in glob.glob("%s/*" %x): #Store list of sessions for current participant
        for files in glob.glob("%s/*" %sessions):
            ...
            # print(part)
            # print(bool(glob.glob("source_images/%s" %part)))
            if(bool(glob.glob("source_images/%s" %part))):
                ...
                os.system('sudo cp ' + sourcefile_neutral + ' ' + dest_neut) #Copy file
                os.system('sudo cp ' + sourcefile_emotion + ' ' + dest_emot) #Copy file